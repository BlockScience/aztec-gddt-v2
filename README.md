...
This repository contains a cadCAD simulation model of the Aztec system under the new specification, where 32 slots are bundled per epoch and must be proven sequentially. If proven, they are then added to the "proven chain" as a batch. The model is up-to-date with the latest multi-proof spec as of March 31, 2025. Several proofs can be submitted for the same (chain of) blocks, and epochs can be proven partially. 
...
The simulation model is written in cadCAD. In this section we will record the overall structure of the cadCAD model. We note that an overview of the initially implemented Aztec System is found in [current specs, Nov. 2024](https://hackmd.io/mgRskzOcTHycxwtcxn4nsA?view#Current-Specs-Description---Nov-2024). 
With the change to multi-proof, all Prover bonds, Prover slashings, Prover Quotes and Prover Search period related logic have been removed and replaced with the latest [design specification](https://github.com/AztecProtocol/engineering-designs/pull/53/files).  
